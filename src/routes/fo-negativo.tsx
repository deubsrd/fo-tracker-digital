...
import { ShieldAlert, Plus, Trash2, Search, CheckCircle2, XCircle, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/fo-negativo")({
  ...
  component: () => <RequireAuth><FoNegativo /></RequireAuth>,
});
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="font-stencil text-xs uppercase tracking-widest text-destructive">Fato Observado Negativo</div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{lista.length} ocorrências disciplinares</p>
        </div>
          ...
          <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Textarea value={descricao} onChange={(e) => setDescricao(e.target.value)} rows={4} required placeholder="Descreva a ocorrência…" />
              </div>
              <Button type="submit" disabled={!alunoSelecionado || createRegistro.isPending} variant="destructive" className="w-full font-stencil tracking-widest disabled:opacity-40">
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">Nenhuma ocorrência registrada.</div>
      ) : (

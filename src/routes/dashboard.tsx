...
import { ShieldCheck, ShieldAlert, Users, Trophy } from "lucide-react";
import { RequireAuth } from "@/components/require-auth";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import {
  alunoPorId, alunos, rankingAlunos, registros, ultimosRegistros,
} from "@/lib/mock-data";
...
function Dashboard() {
  ...
  const ultimas  = ultimosRegistros(6);
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            CFC 2026 · {alunos.length} alunos matriculados
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Top 5 */}
        <Card className="shadow-command lg:col-span-1">
          <CardHeader>
        ...
        <Card className="shadow-command lg:col-span-2">
          <CardHeader>
            <CardTitle className="font-stencil tracking-wider">
              Últimas Ocorrências
            </CardTitle>
          </CardHeader>
...
function KpiCard({
  ...
  return (
    <Card className="shadow-command relative overflow-hidden">
      <div className={`absolute top-0 left-0 h-full w-1 ${barClass}`} />
      <CardContent className="pt-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className={`font-stencil font-bold mt-2 ${small ? "text-lg" : "text-3xl"} ${toneClass}`}>
              {value}
            </div>
          </div>
